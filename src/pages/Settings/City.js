...
function City() {
  ...
  const EditDep = () => {
    ...
    axios
      .post(URLS.UpdateCity, dataArray, {
        headers: { Authorization: `Bearer ${token}` },
      })
      .then(
        res => {
  ...
  const Deletedep = data => {
    var token = datas
    const dataArray = data._id

    axios
      .post(
        URLS.InActiveCity + dataArray,
        {},
        {
          headers: { Authorization: `Bearer ${token}` },
        }
  ...
  const Getalldep = () => {
    ...
    axios
      ...
      .then(
        res => {
          setbanner(res.data.constResult)
        },
        error => {
          if (error.response && error.response.status === 400) {
            toast(error.response.data.message)
  ...
  const getpopup = data => {
    ...
    axios
      .post(URLS.DropdownStates, dataArray, {
        headers: { Authorization: `Bearer ${token}` },
      })
      .then(res => {
  ...
  return (
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          <Breadcrumbs title="Narada Media Admin" breadcrumbItem="City" />
          <Row>
            <Col md={4}>
              <Card>
