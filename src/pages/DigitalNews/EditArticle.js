...
function Addnews() {
  ...
  const [middleImage, setMiddleImage] = useState([]) // New state for middle image
  ...
  const AddPosters = () => {
    ...
    // Append middle image if exists
    if (middleImage.length > 0) {
      for (let i = 0; i < middleImage.length; i++) {
        dataArray.append("middleImage", middleImage[i])
      }
    }

    axios
      .put(URLS.UpdateArticle + "/" + formid, dataArray, {
        headers: { Authorization: `Bearer ${token}` },
      })
      .then(
        res => {
  ...
  const Getslider = () => {
    ...
    axios
      ...
      .then(res => {
        const article = res.data.artcile[0]
        setform({
          ...article,
          allowCopy: article.allowCopy || "both" // Set default if not exists
        })
        setTags(article?.tags || [])
        setSelectedOptions(article?.location || [])
        setSelectedOptions1(article?.category || [])
        setText(article?.allcontent || "")
        setform1(article?.image || [])
        // Set existing middle image if available
        if (article?.middleImage) {
          setMiddleImage([{ 
            preview: URLS.Base + article.middleImage,
            name: "Existing Middle Image",
  ...
  return (
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          <Breadcrumbs title="Narada" breadcrumbItem="Edit Article" />
          <Form
            onSubmit={e => {
              handleSubmit(e)
            }}
